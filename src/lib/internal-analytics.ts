...
type DashboardData = {
  trafficSummary: Record<string, unknown>[];
...
export async function ensureAnalyticsTable() {
  ...
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_site_analytics_events_session_id ON site_analytics_events(session_id);`);
...
export async function getInternalAnalyticsDashboardData(): Promise<DashboardData> {
  await ensureAnalyticsTable();
